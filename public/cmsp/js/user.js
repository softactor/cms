 ...
 */
function OpenUserListUpdateModal(url, row_id){
    $.ajax({
        url         :   url,
        type        :   'GET',
        dataType    :  'json',
        ...
        success     : function(response){
            ...
            $('#userListEdit').modal('show');
            $('#modal_body_content').html(response.data);
        }
    });
}

function udateUserList(url){
    $.ajax({
        url         :  url,
        type        :  'POST',
        dataType    :  'json',
        ...
        data        :  $('#user_list_update_form').serialize(),
        success     :  function(response){
...
function confirmUserDel(url, row_id){
    swal({
        title: "Are you sure?",
